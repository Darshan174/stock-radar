...
async function handleStockScore(request: NextRequest): Promise<NextResponse> {
  ...
  const result = await backendRequest<Record<string, unknown>>("/v1/agent/stock-score", {
    method: "GET",
    timeoutMs: 30000,
...
export async function GET(request: NextRequest) {
  ...
  try {
    return await withX402(request, "/api/agent/stock-score", handleStockScore)
  } catch (error) {
    return backendErrorResponse(error, "Failed to calculate stock score")
  }
}
