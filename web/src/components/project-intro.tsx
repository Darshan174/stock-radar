...
const FEATURES = [
  ...
  },
  {
    ...
    desc: "XMTP client accessed via HTTP wrapper enables capability discovery, pricing negotiation, and automated task delegation. Agents discover, negotiate, pay, and rate each other without human intermediaries.",
  },
  {
    icon: Layers,
    color: "Gold",
  ...
  },
  {
    ...
    desc: "Context-aware, stock-specific AI chat interfaces powered by a robust multi-model fallback chain (Groq → glm-4.7 → Gemini) for conversational financial insights.",
  },
  {
    icon: Radio,
    color: "Teal",
    title: "Proactive Alert Network",
    desc: "Cross-platform notifications delivered via Slack and Telegram with advanced retry logic. Get instant, automated alerts for entries, exits, target-hits, and stop-loss events.",
  },
  {
    icon: BarChart3,
    color: "Gold",
    title: "Comprehensive Dashboards",
    desc: "Actionable RAG Insights panel mapping past similar analyses, advanced technical charts, and an interactive Analytics Dashboard tracking x402 revenue and gasless transaction savings.",
  }
]
...
const AI_ML_FEATURES = [
  {
    ...
    desc: "Production fallback chain with openai/glm-4.7 (primary) → Gemini (secondary). LiteLLM abstraction enables seamless model switching. Structured JSON extraction from LLM responses powers all trading signal generation.",
    details: [
      "openai/glm-4.7 endpoint as primary",
  ...
  },
  {
    ...
    desc: "Retrieval-Augmented Generation using pgvector embeddings injects real-time market context into LLM prompts. A dedicated RAG Retriever fetches relevant earnings data and sector news to ground AI predictions.",
    details: [
      "pgvector embeddings for semantic search capabilities",
  ...
  },
  {
    ...
    desc: "Multi-source sentiment pipeline combining FinBERT transformer model for financial text classification, Finnhub social sentiment scores, and comprehensive earnings call sentiment impact assessment.",
    details: [
      ...
      "Continuous sentiment ingestion into Postgres schema",
    ],
  },
  {
    icon: FlaskConical,
    ...
    desc: "45 engineered features for the ML prediction model (20 base + 6 cross-sectional + 6 factor + 5 microstructure + 8 sentiment), built meticulously via the dataset builder.",
    details: [
      "Technical: RSI, MACD, Bollinger Bands, ATR, ADX",
      "Cross-sectional & factor definitions",
      "Market microstructure transformations",
      "Time-lagged and temporal price dynamics",
    ],
  },
  {
    icon: Crosshair,
    ...
    desc: "Rule-based heuristic scoring (ADX / ATR / RSI thresholds) to classify bull, bear, sideways, and volatile regimes. The router dynamically adjusts scoring presets and risk parameters.",
    details: [
      "Rule-based regime classification from price indicators",
  ...
  },
  {
    ...
    desc: "sklearn GradientBoostingClassifier trained on historical stock data with walk-forward validation. A model registry manages versioning, A/B testing, and canary deployments with rollback on drift.",
    details: [
      "sklearn GradientBoostingClassifier model core",
  ...
  },
  {
    ...
    desc: "Pre-trade risk management checking position sizing against portfolio constraints. Paper trading and a calibration module runs live signals against market data to evaluate predicted vs realized returns.",
    details: [
      ...
      "Broker connection layer for execution routing",
    ],
  },
  {
    icon: ShieldCheck,
    color: "Teal",
    title: "Infrastructure & ReAct Agents",
    desc: "Supabase-backed async job queues (ThreadPoolExecutor), interactive CLI research agents powered by a multi-step ReAct loop, Redis caching, and a full Prometheus + Grafana observability stack.",
    details: [
      "Interactive CLI agents built via a Tool-Calling ReAct loop",
      "Supabase-backed Async Job Queue ensuring non-blocking pipelines",
      "Prometheus & Grafana infrastructure monitoring",
      "Bazaar Indexer mapping Aptos blockchain agent events",
    ],
  },
]
...
const TECH_STACK = [
  ...
  { label: "glm-4.7 / Gemini", icon: Brain },
  { label: "Supabase + pgvector", icon: Database },
  { label: "XMTP Protocol", icon: MessageSquare },
  { label: "Prometheus", icon: Activity },
  { label: "Redis Caching", icon: Database },
  { label: "sklearn GradientBoosting", icon: TrendingUp },
    ...
    <div className={`${styles.introRoot} ${displayFont.variable} ${monoFont.variable}`}>
      ...
      <nav className={styles.nav}>
        ...
        <div className="flex bg-[#0A1118] border border-[#1a2530] text-white rounded-full items-center">
          ...
          <div className="flex gap-6 pl-4 pr-6 text-sm font-medium">
            <button className="hover:text-[#4ade80] transition-colors" onClick={() => scrollTo("features")}>Features</button>
            <button className="hover:text-[#4ade80] transition-colors" onClick={() => scrollTo("aiml")}>AI / ML</button>
            <button className="hover:text-[#4ade80] transition-colors" onClick={() => scrollTo("pricing")}>Pricing</button>
            <button className="hover:text-[#4ade80] transition-colors" onClick={() => scrollTo("tech")}>Tech Stack</button>
          </div>
        </div>
      </nav>
      ...
      {/* ── IDE Showcase ── */}
      <section className={`${styles.showcaseSection} bg-[#22c55e]`} ref={showcase.ref}>
      ...
      <section id="features" className={`${styles.featuresSection} bg-white text-black`} ref={features.ref}>
        ...
        <div className={`${styles.featuresGrid} ${features.className}`}>
          {FEATURES.map((f) => (
            <div key={f.title} className={`${styles.featureCard} !bg-gray-50 !border-gray-200`}>
      ...
      {/* ── Pricing ── */}
      <section id="pricing" className={`${styles.pricingSection} bg-[#22c55e]`} ref={pricing.ref}>
